...
本项目分别维护王栎鑫 2007—2026 公开履历资料库与纪念网站精选时间轴。完整活动档案来自飞书多维表格导出的本地源文件，精选时间轴独立维护，二者不互相生成或覆盖。
...
- `build_wang_yuexin_archive.mjs` 中的 `works` 和 `events` 仍用于生成工作簿交付物，但网站完整活动档案的权威源改为 `sources/feishu/公开活动合集.json`。
...
- `site/data/archive.json` 必须从 `sources/feishu/公开活动合集.json` 生成，不手工编辑。
- `site/data/timeline.json` 不从工作簿生成；资料库刷新不得修改或覆盖它。
- 活动默认按年份、日期、类别和名称升序；同日不同活动分别保留。
- 活动年表类别固定为“影视、音乐、舞台、综艺、直播、杂志、其他”；网站必须直接按该字段分类，不根据活动名称推断。
- 重复活动按“日期 + 活动名称 + 地点”识别。
...
- 现役工作簿为 `王栎鑫2007-2026公开履历资料库_完整版_按年份排序.xlsx`，但网站完整档案已改以飞书多维表格源文件为准。
- 网站读取 `site/data/archive.json` 和 `site/data/timeline.json`。
- 活动年表数据变更后先更新 `sources/feishu/公开活动合集.json`，再运行完整刷新；时间轴数据变更后运行 `node verify_timeline.mjs`，并以对应验证通过为完成标准。
