...
这是王栎鑫 2007—2026 公开履历资料库及纪念网站项目。资料库与网站完整档案现在分离维护：活动年表以飞书多维表格导出的本地 JSON 为权威源，网站精选时间轴使用独立 JSON 维护；Excel 工作簿仍作为交付物保留，但不再作为网站完整档案的源头。
...
├── extract_timeline.mjs           # 从飞书多维表格源文件导出完整档案，不修改时间轴
├── verify_archive.mjs             # 源文件、完整档案与状态文档一致性检查
...
1. 更新 `sources/feishu/公开活动合集.json`，它是网站完整档案的源文件。
2. 如需保持 Excel 工作簿交付物同步，再更新 `build_wang_yuexin_archive.mjs` 的 `works`、`events` 与来源常量。
3. 同步更新 `PROJECT_STATUS.md` 中确实发生变化的数量或核验状态。
4. 运行完整刷新：
...
完整刷新依次执行：生成工作簿、从飞书源文件导出 `archive.json`、验证源文件/完整档案/状态文档一致性，并检查独立时间轴自身结构。该流程不会改写 `timeline.json`。

网页完整档案使用独立展示顺序：年份按2026至2007倒序，同一年内按12月至1月倒序、同月按日期倒序。每条活动直接显示中文日期；资料源只有月份或年份时，按现有精度显示并将年份级待核条目置于该年末尾。

连续参加同一档综艺同一季的不同播出期数，在资料库中合并为一条从首次到末次播出的时间段记录；先导片、直播等不同内容仍独立登记。除此之外，网页不对巡演场次或同名活动进行系列折叠，完整档案中的每条合并履历始终作为独立行展示。网页分类直接读取活动年表的“类别”字段，类别固定为“影视、音乐、舞台、综艺、直播、杂志、其他”。
...
- `sources/feishu/公开活动合集.json`、`archive.json` 与 `PROJECT_STATUS.md` 的数量一致。
- `archive.json` 与飞书源文件逐行一致。
- `timeline.json` 独立验证 32 条节点、必备字段、连续顺序、ID唯一性、日期格式、特殊节点取值及图片存在性，不与活动年表逐行比较。
- 活动 ID 唯一，日期/名称/地点组合无重复，默认排序正确。
- 所有条目必须与 `sources/feishu/公开活动合集.json` 保持一致。
- 活动年表类别只能使用“影视、音乐、舞台、综艺、直播、杂志、其他”。
- 已填充单元格中没有常见公式错误值。
